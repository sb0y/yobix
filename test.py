...
class Pr0cks5Server(asyncore.dispatcher):
	...
	def handle_accept(self):
		...
		if pair is not None:
			...
			#handler = SockConn(sock, verbose=self.verbose)
			...
			right = socket.create_connection((address, port))
			#print("connected to %s:%d" % (address, port))

			a, b = Sock(sock, None), Sock(right, None)
			a.other, b.other = b, a
...
if __name__=='__main__':
	...
	socks.setdefaultproxy(proxytype=None, addr=proxy_addr, port=proxy_port, username=args.username, password=args.password)
